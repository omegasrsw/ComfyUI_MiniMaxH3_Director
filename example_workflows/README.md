...
All long-audio examples expose corrected frames plus full-video `positive`
and `negative` guidance. No latent is exported. Encode corrected frames later
with native VAE Encode and the H3 video VAE, then use the H3 latent upscaler.
See [padding, wiring and conditioning details](../README.md#external-video-encoding-and-refinement).
